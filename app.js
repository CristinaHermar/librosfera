...
// ========= Traducción UI + carga de catálogo =========
function applyTranslations(lang){
  ...
  // ✅ Recargar catálogo con fetch para evitar CORS
  fetch(SHEET_URLS[lang])
    .then(r => r.text())
    .then(csvText => {
    ...
    .catch(err => {
      console.error('Error descargando CSV (fetch):', err);
      CATALOG = FALLBACK_BOOKS;
      renderGenres();
      updateOpts();
      updateVisibility();
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // ✅ Cargar CSV inicial con fetch (según idioma guardado)
  fetch(SHEET_URLS[currentLang])
    .then(r => r.text())
    .then(csvText => {
      const res = Papa.parse(csvText, { header: true, skipEmptyLines: true });
      try {
